...
Pixel_Art_Upsampler_Constant_Buffer constants : register(b0);
Texture2D<float4>                   input     : register(t0);
RWTexture2D<float4>                 output    : register(u0);

[numthreads(PIXEL_ART_UPSAMPLER_WIDTH, PIXEL_ART_UPSAMPLER_HEIGHT, 1)]
void cs_pixel_art_upsampler(uint2 tid : SV_DispatchThreadID)
{
	uint2 sample_tl  = (tid * constants.input_size) / constants.output_size;
	uint2 sample_br  = ((tid + 1) * constants.input_size) / constants.output_size;
	float4 sample_00 = input[sample_tl];
	float4 sample_10 = input[uint2(sample_br.x, sample_tl.y)];
	float4 sample_01 = input[uint2(sample_tl.x, sample_br.y)];
